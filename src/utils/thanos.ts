...
export const disintegrate = ($elm: HTMLElement): void => {
  void html2canvas($elm, {
    useCORS: true, //允许跨域
    allowTaint: true,
  }).then(($canvas) => {
    ...
    $container.classList.add("html2canvas-disintegration-container");
    ...
    const $frameCanvases = frames.map((frameData, i) => {
      const $c: HTMLElement = $canvas.cloneNode(true);

      // 将数据从已有的 ImageData 对象绘制到位图的方法。
      $c.getContext("2d").putImageData(frameData, 0, 0); //eslint-disable-line
    ...
    $elm.classList.add("html2canvas-disintegrated");
